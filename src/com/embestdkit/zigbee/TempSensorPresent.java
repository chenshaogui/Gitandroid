...
public class TempSensorPresent extends NodePresent {
	...
	View mInfoView;
	ThreeXYCurveView mTempCurveView;
	...
	TempSensorPresent(Node n) {
		...
		mTempCurveView = (ThreeXYCurveView) super.mView
				.findViewById(R.id.tempCurveView);
		...
		mEditTextHighter = (EditText) mTempConfigView
				.findViewById(R.id.temp_et_heighter);
		mEditTextLower = (EditText) mTempConfigView
				.findViewById(R.id.temp_et_lower);
		mEditTextNumber = (EditText) mTempConfigView
				.findViewById(R.id.temp_et_number);
		mCheckBox = (CheckBox) mTempConfigView
				.findViewById(R.id.temp_checkbox_enable_alarm);
		// 报警上下限
	...
	void procAppMsgData(int addr, int cmd, byte[] dat) {
		...
		valueh = Tool.builduInt(dat[3]); // dat[2];b&0xff
		mTempCurveView.addData((byte) value, (byte) valueh);// 去初始化 画图
