 ...
 * 
 */
public class ThreeXYCurveView extends View {
	static String TAG = "HumidityCurveView";

	public ThreeXYCurveView(Context context) {
		super(context);

	}

	public ThreeXYCurveView(Context c, AttributeSet attr) {
		super(c, attr);
	 ...
	 * @param style
	 */
	public ThreeXYCurveView(Context c, AttributeSet attr, int style) {
		super(c, attr, style);
	}
	...
	public void addData(byte v, byte vy) {
		// v = 40;
		// vy= 70;
		if (v <= 0)
			v = 0;
	...
	void drawCross(Canvas canvas) {
		...
		canvas.drawText("当前温度值:", spaceWidth + textw / 2, sh + ph / 2 - 50,
				paint);
		if (mDataCnt > 0) {
			...
			canvas.drawText(string, spaceWidth + textw / 2, sh + ph / 2 - 10,
					paint);
		}
		paint.setColor(Color.YELLOW);
		// canvas.drawText("当前Y值", spaceWidth+textw/2, sh+ph/2+50, paint);
		canvas.drawText("当前湿度值:", spaceWidth + textw / 2, sh + ph / 2 + 30,
				paint);
		if (mDataCnty > 0) {
			...
			canvas.drawText(string, spaceWidth + textw / 2, sh + ph / 2 + 70,
					paint);
