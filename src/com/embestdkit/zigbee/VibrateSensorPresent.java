...
public class VibrateSensorPresent extends NodePresent {

	View mInfoView;
	View mConfigView;

	ImageView mVibrateSensorImageView;
	 ...
	 * @param n
	 */
	VibrateSensorPresent(Node n) {
		super(R.layout.vibrate_sensor, n);
		mInfoView = super.mView.findViewById(R.id.vibratesensorInfoView);
		mConfigView = super.mView.findViewById(R.id.vibratesensorConfigView);

		mVibrateSensorImageView = (ImageView) mConfigView.findViewById(R.id.vibratesensorImageView);

		mBtnEnable = (Button) mConfigView.findViewById(R.id.vibratesensorBtnEnable);
		mBtnEnable.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				if (!mSensorEnable) {
					VibrateSensorPresent.super.sendRequest(0x0002, new byte[] {
							0x06, 0x01, 0x01 });
				}
			}
		});
		...
		mBtnDisable = (Button) mConfigView
				.findViewById(R.id.vibratesensorBtnDisable);
		...
		mAlarmNumberEditText = (EditText) mConfigView
				.findViewById(R.id.vibratesensorAlarmNumberEditText);
		mAlarmCheckBox = (CheckBox) mConfigView
				.findViewById(R.id.vibratesensorAlarmCheckBox);
			...
			if (pid == 0x0601) {
				...
				if (dat[i + 2] != 0) {
					...
					if (mAlarmCheckBox.isChecked()) {
						/* 发送告警通知和短信 */
						String msg = this.mNode.mNetAddr + ":检测到振动";
						Tool.notify("振动告警", msg);
						Tool.playAlarm(3);
	...
	@Override
	void setup() {
		super.sendRequest(0x0001, new byte[] { 0x06, 0x01, 0x06, 0x02, });
