 ...
 * 
 */
public class ThreeXLCurveView extends View {
	static String TAG = "HumidityCurveView";

	public ThreeXLCurveView(Context context) {
		super(context);

	}

	public ThreeXLCurveView(Context c, AttributeSet attr) {
		super(c, attr);
	 ...
	 * @param style
	 */
	public ThreeXLCurveView(Context c, AttributeSet attr, int style) {
		super(c, attr, style);
	}
	...
	// short[] mData = new short[MAX_DATA_CNT];
	...
	public void addData(byte v) {
		...
		if (mDataCnt == 0)
			mBaseValue = v;
		// if (mDataCnty == 0) mBaseValuey = vy;
		// if (mDataCntz == 0) mBaseValuez = vz;
		...
		// if (mDataCnty<MAX_DATA_CNTy) mDataCnty++;
		//
		// mDataz[MAX_DATA_CNTz-1] = vz;
		// if (mDataCntz<MAX_DATA_CNTz) mDataCntz++;
	...
	void drawCross(Canvas canvas) {
		...
		canvas.drawText("当前光强度值:", spaceWidth + textw / 2, sh + ph / 2 - 20,
				paint);
		if (mDataCnt > 0) {
			// String str = String.format("%d%", mData[MAX_DATA_CNT-1]);
			// String str = String.format("%d", mData[MAX_DATA_CNT-1]);
			// Short.toString(mData[MAX_DATA_CNT-1]);
			...
			// int a= mData[MAX_DATA_CNT-1]/10;
			// int c= mData[MAX_DATA_CNT-1]%10;
			//
			// char b= (Integer.toString(a)).charAt(0);
			// char d= Integer.toString(c).charAt(2);
			// String e= b+"."+d;
			float f = (float) (mData[MAX_DATA_CNT - 1] * 0.1);
			Float.toString(f);
			// canvas.drawText(Short.toString(mData[MAX_DATA_CNT-1]),
			// spaceWidth+textw/2, sh+ph/2-30, paint);
			canvas.drawText(Float.toString(f) + "V", spaceWidth + textw / 2, sh
					+ ph / 2 + 20, paint);

		}
		paint.setColor(Color.YELLOW);
		// canvas.drawText("当前Y值", spaceWidth+textw/2, sh+ph/2+50, paint);
		// canvas.drawText("当前Y值:", spaceWidth+textw/2, sh+ph/2+10, paint);
		// if (mDataCnty > 0) {
		...
		// canvas.drawText(string, spaceWidth+textw/2, sh+ph/2+50, paint);
		//
		// }
		// paint.setColor(Color.CYAN);
		// canvas.drawText("当前Z值", spaceWidth+textw/2, sh+ph/2+120, paint);
		// canvas.drawText("当前Z值:", spaceWidth+textw/2, sh+ph/2+ 90, paint);
		// if (mDataCntz > 0) {
		...
		// canvas.drawText(string, spaceWidth+textw/2, sh+ph/2+130, paint);
		//
		// }
		/* 曲线矩形框 */
		paint.setStrokeWidth(2);
		...
		canvas.drawText(String.format("%d", 5), mXStart + cw * 2 + 2,
				mYStart + 8, paint);
		//
		// }
		...
		int x1 = mXStart, y1 = mYStart - (mData[MAX_DATA_CNT - 1] - mBaseValue)
				* dy / 5;
		int x2, y2;
		// int x1y=mXStart, y1y=mYStart - (mDatay[MAX_DATA_CNTy-1] -
		// mBaseValuey)*dy / 5;
		// int x2y, y2y;
		// int x1z=mXStart, y1z=mYStart - (mDataz[MAX_DATA_CNTz-1] -
		// mBaseValuez)*dy / 5;
		// int x2z, y2z;
		...
		for (int i = 1; i < mDataCnt; i++) {
			...
			// x2y = mXStart - i * dx;
			// y2y = mYStart - (mDatay[MAX_DATA_CNTy-i-1] - mBaseValuey)*dy / 5;
			// paint.setColor(Color.YELLOW);
			// canvas.drawLine(x1y,y1y, x2y, y2y, paint);
			// x1y = x2y; y1y = y2y;

			// x2z = mXStart - i * dx;
			// y2z = mYStart - (mDataz[MAX_DATA_CNTz-i-1] - mBaseValuez)*dy / 5;
			// paint.setColor(Color.CYAN);
